...
public class EffectiveAlarmConsumer extends JAWSConsumer<String, EffectiveAlarm> {
    ...
    private static Properties setDefaults(Properties overrides) {
        ...
        defaults.put(EventSourceConfig.GROUP_ID_CONFIG, "effective-alarm-consumer" + Instant.now().toString() + "-" + Math.random());
        defaults.put(EventSourceConfig.TOPIC_CONFIG, EffectiveAlarmProducer.TOPIC);
